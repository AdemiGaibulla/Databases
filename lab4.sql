...
GROUP BY employees.employee_id, employees.first_name, employees.last_name, employees.department
...
SELECT department,
       ...
       GREATEST(AVG(salary) - 10000, 0) AS lower_bound_demo,
       LEAST(MAX(salary) + 10000, 1000000) AS upper_bound_demo
FROM employees
GROUP BY department;
